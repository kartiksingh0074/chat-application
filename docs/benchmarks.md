...
| Every answer carries citations, each a real message in the room (§8.7 criterion 4) | **Confirmed after a fix.** Every saved citation resolves to a real message in its own room, and a question the history cannot answer gets none rather than an invented one. But the first check counted only citations that were saved, so it missed an answer that saved none: `gpt-oss` wrote `【1】` instead of `[1]` and the parser only read ASCII brackets. It now reads both; that one earlier answer keeps no citations, since its sources were not stored |
